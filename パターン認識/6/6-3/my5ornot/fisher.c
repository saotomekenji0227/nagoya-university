...
void main(int argc,char *argv[]){
  ...
  for(i = FIRST; i <= LAST; i++){
    ...
    sprintf(fileName,"./081%6d/glucksman.dat",MYNUM);
    //自分のデータも読み込み
    //次元圧縮を行うため、毎回読み込んでいる
    readFile(fileName,&w1);
    //ここからの処理は出力する内容が減ったため
    //いらない出力を削った以外変更なし
...
void writeAnswer(Class *class1, Class *class2, double *A,int a,int b){
  ...
  int i,j;
  double *tmp;
  sprintf(fileName,"fisher%06d-%06d.dat",a,b);
  fp = fopen(fileName,"w");
  tmp = (double*)malloc(class1->div * sizeof(double));
  for(i = 0; i < class1->datanum; i++){
    for(j = 0; j < class1->div; j++)
